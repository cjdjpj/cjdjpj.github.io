...
Two alleles **coalesce** when their lineages merge (i.e. they reach a common ancestor). There are two exhaustive possibilities for the ancestry of two alleles:
...
When trying to estimate $$\pi$$, it is important to note that we are now operating at the level of individuals with potentially many alleles. Let's call $$T_2$$ the time until two samples coalesce.
